...
import { ImportTemplate, ColumnMapping, ParsedFileColumn, FileParseResult, DrugType, STANDARD_ORDER_FIELDS } from "@/types/import";
...
export function TemplateBuilderSheet({
  ...
  const content = <div className="fixed inset-0 z-50 flex flex-col bg-background overflow-hidden w-full max-w-full">
      {/* Sticky Header */}
      <header className="sticky top-0 z-10 flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 shrink-0 w-full max-w-full">
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          ...
          {templateName && <p className="text-xs sm:text-sm text-muted-foreground truncate">{templateName}</p>}
        </div>
        <Button onClick={handleSave} disabled={!templateName.trim() || isSaving} size={isMobile ? "sm" : "default"} className="shrink-0">
          {isSaving ? "Saving..." : "Save"}
        </Button>
      </header>

      {/* Scrollable Content */}
      <div className="flex-1 w-full max-w-full overflow-y-auto overscroll-contain px-[11px]">
        <div className={cn("mx-auto pb-8 w-full max-w-full box-border", isMobile ? "px-2 py-3" : "max-w-4xl px-6 py-6")}>
          <div className="space-y-4 w-full max-w-full overflow-hidden">
            {/* Section 1: Template Information */}
            <Collapsible open={sectionsOpen.info} onOpenChange={() => toggleSection("info")}>
              <Card className="border-border">
                <CollapsibleTrigger asChild>
                  <div className={cn("flex items-center justify-between cursor-pointer hover:bg-muted/50 transition-colors", isMobile ? "p-3 gap-2" : "p-4 gap-3")}>
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                      <div className={cn("rounded-full bg-primary/10 flex items-center justify-center shrink-0", isMobile ? "w-6 h-6" : "w-8 h-8")}>
                    ...
                    {sectionsOpen.info ? <ChevronUp className={cn("text-muted-foreground shrink-0", isMobile ? "h-4 w-4" : "h-5 w-5")} /> : <ChevronDown className={cn("text-muted-foreground shrink-0", isMobile ? "h-4 w-4" : "h-5 w-5")} />}
                  </div>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <CardContent className={cn("pt-0 space-y-4", isMobile ? "pb-3 px-3" : "pb-4 px-4")}>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Input id="templateName" value={templateName} onChange={e => setTemplateName(e.target.value)} placeholder="e.g., Naloxone Kit Template" className={isMobile ? "h-9 text-sm" : ""} />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="description" className={isMobile ? "text-sm" : ""}>
                          Description
                        </Label>
                        <Input id="description" value={description} onChange={e => setDescription(e.target.value)} placeholder="Optional description" className={isMobile ? "h-9 text-sm" : ""} />
                      </div>
                    </div>

                    <div className={cn("flex items-center justify-between bg-muted/50 rounded-lg", isMobile ? "p-2.5" : "p-3")}>
                      <div className="min-w-0">
                ...
                <CollapsibleTrigger asChild>
                  <div className={cn("flex items-center justify-between cursor-pointer hover:bg-muted/50 transition-colors", isMobile ? "p-3 gap-2" : "p-4 gap-3")}>
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                      {fileName && <Badge variant="secondary" className={cn("gap-1", isMobile && "text-[10px] px-1.5 py-0")}>
                          <FileSpreadsheet className={isMobile ? "w-2.5 h-2.5" : "w-3 h-3"} />
                          Loaded
                        </Badge>}
                      {sectionsOpen.file ? <ChevronUp className={cn("text-muted-foreground", isMobile ? "h-4 w-4" : "h-5 w-5")} /> : <ChevronDown className={cn("text-muted-foreground", isMobile ? "h-4 w-4" : "h-5 w-5")} />}
                    </div>
                  </div>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <CardContent className={cn("pt-0", isMobile ? "pb-3 px-3" : "pb-4 px-4")}>
                    ...
                    {!fileName ? <div className={cn("border-2 border-dashed border-border rounded-lg text-center cursor-pointer hover:border-primary/50 hover:bg-muted/30 transition-colors", isMobile ? "p-5" : "p-8")} onClick={() => fileInputRef.current?.click()}>
                      ...
                      </div> : <div className={cn("flex items-center justify-between bg-primary/5 border border-primary/20 rounded-lg", isMobile ? "p-3 gap-2" : "p-4 gap-3")}>
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                          <div className={cn("rounded-lg bg-primary/10 flex items-center justify-center shrink-0", isMobile ? "w-8 h-8" : "w-10 h-10")}>
                          ...
                          <div className="min-w-0">
                            ...
                            {parseResult && <p className={cn("text-muted-foreground", isMobile ? "text-xs" : "text-sm")}>
            ...
            {parseResult && parseResult.columns.length > 0 && <Collapsible open={sectionsOpen.mapping} onOpenChange={() => toggleSection("mapping")}>
                <Card className="border-border">
                  <CollapsibleTrigger asChild>
                    <div className={cn("flex items-center justify-between cursor-pointer hover:bg-muted/50 transition-colors", isMobile ? "p-3 gap-2" : "p-4 gap-3")}>
                      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        {mappedCount > 0 && <Badge className={cn("gap-1 bg-orange-100 text-orange-700 hover:bg-orange-200 dark:bg-orange-900/30 dark:text-orange-400 border-0", isMobile && "text-[10px] px-1.5 py-0")}>
                            <Check className={isMobile ? "w-2.5 h-2.5" : "w-3 h-3"} />
                            {mappedCount} mapped
                          </Badge>}
                        {skippedCount > 0 && <Badge variant="secondary" className={cn("gap-1", isMobile && "text-[10px] px-1.5 py-0")}>
                            {skippedCount} skipped
                          </Badge>}
                        {sectionsOpen.mapping ? <ChevronUp className={cn("text-muted-foreground", isMobile ? "h-4 w-4" : "h-5 w-5")} /> : <ChevronDown className={cn("text-muted-foreground", isMobile ? "h-4 w-4" : "h-5 w-5")} />}
                      </div>
                    </div>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                    <CardContent className={cn("pt-0 overflow-hidden w-full max-w-full", isMobile ? "pb-3 px-4" : "pb-4 px-4")}>
                      ...
                      {mappedColumns.length > 0 && <div className="mb-4 sm:mb-6">
                          ...
                          <div className={cn("grid gap-2 sm:gap-3 w-full max-w-full", isMobile ? "grid-cols-1" : "grid-cols-2 lg:grid-cols-3")}>
                            {mappedColumns.map(column => <ColumnMappingCard key={column.name} column={column} mappedTo={columnMappings.get(column.name) || "skip"} onMappingChange={handleMappingChange} drugTypes={drugTypes} usedFields={usedFields} isMobile={isMobile} />)}
                          </div>
                        </div>}

                      {/* Unmapped Columns Section */}
                      {unmappedColumns.length > 0 && <div>
                          ...
                          <div className={cn("grid gap-2 sm:gap-3 w-full max-w-full", isMobile ? "grid-cols-1" : "grid-cols-2 lg:grid-cols-3")}>
                            {unmappedColumns.map(column => <ColumnMappingCard key={column.name} column={column} mappedTo={columnMappings.get(column.name) || "skip"} onMappingChange={handleMappingChange} drugTypes={drugTypes} usedFields={usedFields} isMobile={isMobile} />)}
                          </div>
                        </div>}
                    </CardContent>
                  </CollapsibleContent>
            ...
            {parseResult && mappedCount > 0 && <Collapsible open={sectionsOpen.preview} onOpenChange={() => toggleSection("preview")}>
                <Card className="border-border">
                  <CollapsibleTrigger asChild>
                    <div className={cn("flex items-center justify-between cursor-pointer hover:bg-muted/50 transition-colors", isMobile ? "p-3 gap-2" : "p-4 gap-3")}>
                      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                      ...
                      {sectionsOpen.preview ? <ChevronUp className={cn("text-muted-foreground shrink-0", isMobile ? "h-4 w-4" : "h-5 w-5")} /> : <ChevronDown className={cn("text-muted-foreground shrink-0", isMobile ? "h-4 w-4" : "h-5 w-5")} />}
                    </div>
                  </CollapsibleTrigger>
                  <CollapsibleContent>
                    <CardContent className={cn("pt-0", isMobile ? "pb-3 px-3" : "pb-4 px-4")}>
