...
export function OrderDetailSheet({ order, isOpen, onClose, onUpdate, isAdmin = false }: OrderDetailSheetProps) {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'PENDING':
        return { label: 'Pending', variant: 'secondary' as const, className: 'bg-amber-100 text-amber-800 border-amber-200' };
      case 'CONFIRMED':
        return { label: 'Confirmed', variant: 'secondary' as const, className: 'bg-blue-100 text-blue-800 border-blue-200' };
      case 'IN_ROUTE':
        return { label: 'In Route', variant: 'secondary' as const, className: 'bg-purple-100 text-purple-800 border-purple-200' };
      case 'ARRIVED':
        return { label: 'Arrived', variant: 'secondary' as const, className: 'bg-indigo-100 text-indigo-800 border-indigo-200' };
      case 'COMPLETED':
        return { label: 'Completed', variant: 'secondary' as const, className: 'bg-emerald-100 text-emerald-800 border-emerald-200' };
      case 'REQUEST_ADDRESS_REVIEW':
        return { label: 'Address Review', variant: 'destructive' as const, className: 'bg-red-100 text-red-800 border-red-200' };
      default:
          ...
          <DrawerHeader className="border-b border-border pb-4">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                {order.shipment_id && (
                  <p className="text-sm font-mono text-muted-foreground">{order.shipment_id}</p>
                )}
              </div>
              ...
              <section>
                ...
                <div className="bg-muted/30 rounded-xl p-4">
                  <div className="space-y-2">
                    {[
                      { label: 'Pending', time: order.pending_at },
                      { label: 'Confirmed', time: order.confirmed_at },
                      { label: 'In Route', time: order.in_route_at },
