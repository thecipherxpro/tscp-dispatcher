...
import { ArrowLeft, Package, User, MapPin, Phone, Calendar, Truck, CheckCircle, Navigation, AlertTriangle, Download, Copy, FileText, Clock, UserPlus, Tag, Mail, Syringe, Wind, Pill, FileDown } from 'lucide-react';
...
export default function OrderEditDetail() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      ...
      if (data.assigned_driver_id) {
        const {
          data: driverData
        } = await supabase.from('profiles').select('id, full_name, driver_id, phone').eq('id', data.assigned_driver_id).maybeSingle();
        if (driverData) {
      ...
      const {
        data: logsData
      } = await supabase.from('order_audit_logs').select('id, action, created_at, user_full_name, new_status').eq('order_id', data.id).order('created_at', {
        ascending: false
      }).limit(5);
      if (logsData) {
  ...
  const handleAssignSystemDriver = async () => {
    ...
    try {
      ...
      const {
        data: driverData,
        error: driverError
      } = await supabase.from('profiles').select('id, full_name, driver_id, phone').eq('driver_id', SYSTEM_DRIVER_ID).maybeSingle();
      ...
      if (!shipmentId || !trackingId) {
        const {
          data: shipmentIdData
        } = await supabase.rpc('generate_shipment_id');
        shipmentId = shipmentIdData as string;
        const {
          data: trackingIdData
        } = await supabase.rpc('generate_tracking_id');
      ...
      const {
        data: initialsData
      } = await supabase.rpc('get_client_initials', {
      ...
      await supabase.from('public_tracking').upsert({
        tracking_id: trackingId,
        tracking_url: trackingUrl,
        shipment_id: shipmentId,
        order_id: order.id,
  ...
  const handleSaveDelivery = async () => {
    ...
    try {
      ...
      await supabase.from('public_tracking').update(publicUpdates).eq('order_id', order.id);
  ...
  return <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-background border-b">
        <div className="flex items-center justify-between px-4 lg:px-8 h-14 lg:h-16 max-w-7xl mx-auto w-full">
          <div className="flex items-center gap-3">
            ...
            <Button variant="ghost" size="icon" onClick={() => navigate('/admin/order-edits')}>
          ...
          <Badge className={`text-xs lg:text-sm ${isCompleted ? order.timeline_status === 'COMPLETED_DELIVERED' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' : isAssigned ? 'bg-blue-100 text-blue-800' : 'bg-amber-100 text-amber-800'}`}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
            ...
            <Card className="lg:col-span-2">
                  ...
                  <CardContent className="space-y-3 lg:space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {order.shipment_id && <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => copyToClipboard(order.shipment_id!)}>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {order.tracking_id && <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => copyToClipboard(order.tracking_id!)}>
                            <Copy className="h-3.5 w-3.5" />
                          </Button>}
                      </div>
                    </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3 lg:space-y-4">
                    ...
                    {order.email && <div className="flex items-center gap-3">
                ...
                <Card className="lg:col-span-2">
                  ...
                  <CardContent className="space-y-4">
                    {order.authorizing_doctor_name && <div className="flex items-center gap-3 pb-3 border-b">
                    ...
                    {/* Nasal Spray */}
                    {(order.nasal_drug_name || order.nasal_qty) && <div className="bg-muted/50 rounded-lg p-4 space-y-3">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          {order.nasal_package && <div className="col-span-2">
                    ...
                    {/* Injectable */}
                    {(order.injection_drug_name || order.injection_qty) && <div className="bg-muted/50 rounded-lg p-4 space-y-3">
                        <div className="flex items-center gap-2">
                    ...
                    <div className="relative pl-8">
                      ...
                      <div className="relative pb-6">
                        ...
                        <div className="ml-4">
                          ...
                          {order.picked_up_at && driver && <p className="text-xs text-primary mt-0.5">Assigned to: {driver.full_name} ({driver.driver_id})</p>}
                        </div>
                      </div>
                      ...
                      <div className="relative pb-6">
                        ...
                        <div className="ml-4">
                          ...
                          {order.confirmed_at && driver && <p className="text-xs text-primary mt-0.5">Confirmed by: {driver.driver_id}</p>}
                        </div>
                      </div>
                      ...
                      <div className="relative pb-6">
                        ...
                        <div className="ml-4">
                          ...
                          {(order.shipped_at || order.in_route_at) && driver && <p className="text-xs text-primary mt-0.5">In route by: {driver.driver_id}</p>}
                        </div>
                      </div>

                      {/* Delivered / Completed */}
                      <div className="relative">
                        <div className={`absolute left-[-20px] w-6 h-6 rounded-full flex items-center justify-center ${order.completed_at ? order.timeline_status === 'COMPLETED_DELIVERED' ? 'bg-green-100' : 'bg-red-100' : 'bg-muted'}`}>
                          <Truck className={`h-3 w-3 ${order.completed_at ? order.timeline_status === 'COMPLETED_DELIVERED' ? 'text-green-600' : 'text-red-600' : 'text-muted-foreground'}`} />
                        </div>
                        <div className="ml-4">
                          ...
                          {order.delivery_status && <Badge className={`mt-1 text-xs ${order.timeline_status === 'COMPLETED_DELIVERED' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                              {order.delivery_status.replace(/_/g, ' ')}
                            </Badge>}
                          {order.completed_at && driver && <p className="text-xs text-primary mt-0.5">Completed by: {driver.driver_id}</p>}
                        </div>
                      </div>
                    </div>
                  </CardContent>
                ...
                {driver && <Card>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      {driver.phone && <div className="flex items-center gap-2">
                          <Phone className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">{driver.phone}</span>
                        </div>}
                      {order.picked_up_at && <div className="flex items-center gap-2">
                ...
                <Card className="lg:col-span-2">
                  ...
                  <CardContent>
                    {auditLogs.length === 0 ? <p className="text-sm text-muted-foreground">No activity recorded</p> : <div className="space-y-2">
                        {auditLogs.map(log => <div key={log.id} className="flex items-center justify-between text-sm py-2 border-b last:border-0">
                            <div>
        ...
        <TabsContent value="edit" className="flex-1 overflow-y-auto pb-24 lg:pb-20 mt-0">
                    ...
                    <CardContent className="space-y-6 lg:space-y-8">
                      ...
                      <div className="flex gap-4 lg:gap-6">
                        ...
                        <div className="flex-1 pb-6 lg:pb-8">
                          ...
                          <Input type="datetime-local" value={editPickedUpAt} onChange={e => setEditPickedUpAt(e.target.value)} className="max-w-xs lg:max-w-sm" />
                        </div>
                      </div>
                      ...
                      <div className="flex gap-4 lg:gap-6">
                        ...
                        <div className="flex-1 pb-6 lg:pb-8">
                          ...
                          <Input type="datetime-local" value={editShippedAt} onChange={e => setEditShippedAt(e.target.value)} className="max-w-xs lg:max-w-sm" />
                      </div>
                    </div>
                      ...
                      <div className="flex gap-4 lg:gap-6">
                        <div className="flex flex-col items-center">
                          <div className={`h-10 w-10 lg:h-12 lg:w-12 rounded-full flex items-center justify-center ${editDeliveryStatus ? deliveryOutcomes.find(o => o.value === editDeliveryStatus)?.success ? 'bg-green-100' : 'bg-red-100' : 'bg-muted'}`}>
                            <Truck className={`h-5 w-5 lg:h-6 lg:w-6 ${editDeliveryStatus ? deliveryOutcomes.find(o => o.value === editDeliveryStatus)?.success ? 'text-green-600' : 'text-red-600' : 'text-muted-foreground'}`} />
                          </div>
                        </div>
                        <div className="flex-1">
                          ...
                          <div className="space-y-3 lg:space-y-4">
                            <Select value={editDeliveryStatus} onValueChange={setEditDeliveryStatus}>
                              ...
                              <SelectContent>
                                {deliveryOutcomes.map(outcome => <SelectItem key={outcome.value} value={outcome.value}>
                                    {outcome.label}
                                  </SelectItem>)}
                              </SelectContent>
                            </Select>
                            <Input type="datetime-local" value={editCompletedAt} onChange={e => setEditCompletedAt(e.target.value)} className="max-w-xs lg:max-w-sm" placeholder="Completion time" />
                          </div>
                        </div>
                      </div>
                    </CardContent>
          ...
          {/* Sticky Save Button */}
          {isAssigned && <div className="fixed bottom-16 lg:bottom-0 left-0 right-0 p-4 lg:p-6 bg-background border-t">
