...
const getStatusConfig = (status: string) => {
  switch (status) {
    case 'PENDING':
      return { label: 'Pending', className: 'border-amber-500/50 bg-amber-500/10 text-amber-600 dark:text-amber-400' };
    case 'PICKED_UP_AND_ASSIGNED':
      return { label: 'Assigned', className: 'border-blue-500/50 bg-blue-500/10 text-blue-600 dark:text-blue-400' };
    case 'REVIEW_REQUESTED':
      return { label: 'Review', className: 'border-amber-500/50 bg-amber-500/10 text-amber-600 dark:text-amber-400' };
    case 'CONFIRMED':
      return { label: 'Confirmed', className: 'border-indigo-500/50 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400' };
    case 'IN_ROUTE':
      return { label: 'In Route', className: 'border-purple-500/50 bg-purple-500/10 text-purple-600 dark:text-purple-400' };
    case 'COMPLETED_DELIVERED':
      return { label: 'Delivered', className: 'border-emerald-500/50 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' };
    case 'COMPLETED_INCOMPLETE':
      return { label: 'Incomplete', className: 'border-destructive/50 bg-destructive/10 text-destructive' };
    default:
...
export default function DriverScan() {
  ...
  const handleBarcodeSearch = (barcode: string) => {
    ...
    const foundOrder = orders.find(order => 
      order.tracking_id?.toUpperCase() === trimmedBarcode ||
      order.shipment_id?.toUpperCase() === trimmedBarcode
    );
        ...
        <SheetContent side="bottom" className="h-[85vh] rounded-t-3xl">
          ...
          {selectedOrder && (
            <div className="overflow-y-auto h-[calc(100%-8rem)] py-4 space-y-4">
              ...
              {/* Timestamps */}
              {(selectedOrder.assigned_at || selectedOrder.completed_at) && (
                <Card className="p-4">
